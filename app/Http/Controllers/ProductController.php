...
class ProductController extends Controller
{
    ...
    public function show(string $productHandle)
    {
        ...
        $supplier = Supplier::where('supplier_name', '=', $product->supplier_name, false)->firstOrFail();
        $category = ProductCategory::where('category_id', '=', $product->category_id, false)->firstOrFail();
        $product->supplier_name = $supplier->display_name;
        $attributes = $product->attributes;
        foreach ($attributes['properties'] as $propertyName => $property) {
            $propertyDisplayName =
                $category->filters[$propertyName]['displayName']
                ?? $propertyName;
